...
class TestDreamDaemon(unittest.TestCase):
    ...
    def test_end_to_end_mcp_tool_prototype_pollution_cycle_and_attestation(self):
        import hashlib
        import hmac
        import json
        import tempfile
        ...
        def mock_llm(prompt: str, temp: float) -> str:
            return json.dumps({
                "dream_id": "dream_mcp_proto_pollution",
                "harness_id": "mcp_server_fuzzer",
                "parameters": {
                    "method": "tools/call",
                    "uri": "file:///app/safe.txt",
                    "tool_name": "system_exec",
                    "tool_args": json.dumps({"__proto__": {"polluted": True}}),
                    "payload_size_kb": 1,
                    "timeout_s": 0.5,
                },
                "expected": "statutory_veto_reached",
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            try:
                with patch("core.dream_daemon.schedule_next_dream", return_value={
                    "mode": "telemetry_perturbation",
                    "seed_data": {"harness_id": "mcp_server_fuzzer", "raw_residue": "pollution_candidate"},
                }):
                    res = daemon.run_cycle()
                ...
                self.assertEqual(attestation["payload"]["harness_id"], "mcp_server_fuzzer")
                self.assertEqual(attestation["payload"]["decision"], "promote_candidate")
                self.assertIn("tool_args", attestation["payload"]["parameters"])
